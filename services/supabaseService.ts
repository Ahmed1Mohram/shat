...
export class SupabaseService {
  ...
  async getMessages(conversationId: string): Promise<Message[]> {
    const { data } = await supabase
      ...
      .order('created_at', { ascending: false }) // Fetch latest first to apply limit
      .limit(50);

    // Reverse memory array to chronological order for UI
    const results = data ? [...data].reverse() : [];
    return results.map(this.mapMessage.bind(this));
  }
