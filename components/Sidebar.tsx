...
export const Sidebar: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-transparent relative overflow-hidden">
            {/* Refined Header */}
            <div className="px-4 md:px-6 pt-4 md:pt-8 pb-3 md:pb-4 relative z-10 safe-top">
                    ...
                    {['chats', 'people', 'stories'].map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex-1 py-2.5 text-[12px] font-semibold uppercase tracking-widest rounded-lg transition-all relative ${activeTab === tab
                            ...
                            {searchResults.map(result => (
                                <div key={result.id} className="p-3 rounded-2xl bg-white/50 dark:bg-dark-card/50 hover:bg-white dark:hover:bg-dark-card border border-transparent hover:border-gray-100 dark:hover:border-white/10 flex items-center gap-3 transition-all cursor-pointer">
                                    ...
                                    <button
                                        ...
                                        className={`p-2.5 rounded-full shadow-sm transition-transform active:scale-90 ${result.friendshipStatus === 'accepted' || result.friendshipStatus === 'self'
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white dark:bg-dark-surface w-full max-w-sm rounded-3xl p-6 shadow-2xl relative overflow-hidden border border-gray-200 dark:border-gray-800">
                        ...
                        <div className="flex flex-col items-center gap-4 relative mt-8">
                            ...
                            <div className="w-full mt-2 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <span className="text-lg">🔔</span>
                                        <div>
                                            <p className="text-sm font-semibold text-gray-900 dark:text-white">Notification Preview</p>
                                            <p className="text-[10px] text-gray-500 dark:text-gray-400">Show sender & message in notifications</p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => setShowNotificationContent(!showNotificationContent)}
                                        className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${showNotificationContent ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                                    >
                                        <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-md transition-transform duration-200 ${showNotificationContent ? 'translate-x-[22px]' : 'translate-x-0.5'}`} />
                                    </button>
                                </div>
                            </div>
                        </div>
